...
Cast::Cast (const std::string& caster_id,
std::unique_ptr<spells::SpellHandlerI> _spell_handler,
double _cast_time)
...
std::ostream& os) {
    ...
    event_queue.push (
    events::Event (state.time + std::max<double> (gcd - spell_handler->cast_time, 0),
...
std::ostream& os) {
    ...
    std::make_unique<::effects::RemoveISB> (caster_id);
    state.debuffs.isb_addr = effect.get ();
    os << state.time << ": ISB refreshed.\n";
    event_queue.push (events::Event (state.time + 12, std::move (effect)));
}

CorruptionTick::CorruptionTick (std::string _caster_id, int _rank, double time, double _snapshot_tick_dmg, bool _reset_after)
: EffectI (_caster_id), rank (_rank), initial_cast_time (time),
  snapshot_tick_dmg (_snapshot_tick_dmg), reset_after (_reset_after) {
}
...
std::ostream& os) {
    if (state.debuffs.corruption_ids[caster_id] == initial_cast_time) {
        ...
        os << state.time << ": " << this->caster_id << " dealt "
           << snapshot_tick_dmg << " with a corruption tick.\n";
...
: EffectI (_caster_id), spell_name (_spell_name), dmg (_dmg),
  cast_point_result (_cast_point_result) {
}
...
state::State& state,
std::ostream& os) {
    bool nightfall_proc = (((double)rand () / (RAND_MAX)) <=
    state.casters[this->caster_id].talents.nightfall_proc);
    if (nightfall_proc) {
        os << " nightfall proc.\n";
