...
void ShadowboltSpellHandler::operator()(std::priority_queue<events::Event, std::vector<events::Event>,
                                        ...
                                        state::State& state, logging::CombatLog& log)
{
  ...
  caster_state.mana -= SpellMetadata::mana_cost[rank];
  ...
  {
    log.addLogEvent(
        state.time, this->caster_id, std::string(" casted R" + std::to_string(rank) + " Shadowbolt; hit."));
    ...
    {
      event_queue.push(
          events::Event(state.time + 0.01, std::make_unique<::effects::ApplyISB>(this->caster_id)));
    }
    else
    {
