...
void CorruptionSpellHandler::operator()(std::priority_queue<events::Event, std::vector<events::Event>,
                                        ...
                                        state::State& state, logging::CombatLog& log)
{
  ...
  if (result.hit)
  {
    ...
    log.addLogEvent(
        state.time, this->caster_id, std::string(" casted R" + std::to_string(rank) + " Corruption; hit."));
